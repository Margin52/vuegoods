...
(function(root, factory){
    ...
    var AppConfig = {
        ...
        // 唤起失败时的跳转链接
        FAILBACK: {
            ANDROID: "http://a.app.qq.com/o/simple.jsp?pkgname=com.icocopie.app&fromcase=40003",
            IOS:"https://itunes.apple.com/cn/app/%20%E7%8E%AF%E7%90%83%E8%BE%A3%E5%A6%88/id1261733947?l=zh&ls=1&mt=8"
        },
    ...
    var isAndroidChrome = (ua.match(/Chrome\/([\d.]+)/) || ua.match(/CriOS\/([\d.]+)/))
                          && browser.isAndroid() && !browser.isMobileQQ()&& !browser.isUC();
    ...
    return {
        ...
        generateSchema: function(schema) {
            ...
            if (isAndroidChrome) {
                ...
                console.log('我是安卓chrome浏览器')
                schemaStr =  "intent://" + AppConfig.APK_INFO.PKG + '?url=' + schemaStr +"#Intent;"  +
                ...
                schemaStr = AppConfig.PROTOCAL + "://" + AppConfig.APK_INFO.PKG + '?url=' + schemaStr;
            } else {
        ...
        loadSchema: function(config){
            ...
            console.log('当前schemaUrl', schemaUrl)
